...
class AngleSimpleLinear(nn.Module):
    ...
    def forward(self, x):
        cos_theta = F.normalize(x.view(x.size(0), -1), dim=1).mm(F.normalize(self.weight, p=2, dim=0))
...
class ADVAMSoftmaxLoss:
    def __init__(self, ams):
        self.ams = ams

    def __call__(self, model, inp, targ):
        logits = model(inp)
        return self.ams(logits, targ), logits
